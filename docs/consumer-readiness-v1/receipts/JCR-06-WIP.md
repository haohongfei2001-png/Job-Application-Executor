...
- Duplicate generic selectors do not write the first repeated row. Same-title canonical project records require distinct `record_id` bindings. An observed but unparsed resume research/project section blocks project coverage. English research/project headings are classified.
...
D-01–D-22 are not certified as a set. Production repeated-row add/delete/reorder recovery, controlled React/Vue and virtualized list fixtures, dependency-sensitive location/major drivers, supported site upload completion receipts, multi-page server draft and revision proof, fault injections around every write, and real platform evidence remain. The generic unsupported paths are blocked. DFG-002 browser write reconciliation is engineering debt; it is not a final-live owner item. No owner action or automated final submit is authorized by this checkpoint.
