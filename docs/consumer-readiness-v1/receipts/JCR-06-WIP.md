...
- Explicit site `data-depends-on` metadata now orders a parent control before its dependent field even when DOM order is reversed; the child is located again after redraw, and the option set participates in the observation digest. Missing, cyclic or unresolved parent dependencies block instead of filling a stale child. This is isolated synthetic support for an explicit dependency contract, not general proof for unannotated real ATS controls.
...
Local isolated/headless full suite after the dependency ordering repair: **410 passed in 83.81s**. The focused regression covering the four historical failures and JCR-06 forms passed **25 tests**. The attachment read-error classification passed its focused regression. GitHub exact-head CI and review status must be read from PR #16 for the current head; earlier heads passed both CI jobs.

## Still open

D-01–D-22 are not certified as a set. Production repeated-row add/delete/reorder recovery, controlled React/Vue and virtualized list fixtures, unannotated site dependency drivers, supported site upload completion receipts, broader multi-page recovery and per-action fault injections, and real platform evidence remain. The generic unsupported paths are blocked. DFG-002 browser write reconciliation is engineering debt; it is not a final-live owner item. No owner action or automated final submit is authorized by this checkpoint.
