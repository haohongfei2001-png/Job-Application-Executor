...
- Heads `ec5e109`, `e9650be` and `f1c245a` also passed both exact-head CI jobs. The current local follow-up corrects the chat UI copy and deterministic reply when someone asks to apply through chat but no task was created; the structured form remains the safe entry until JCR-03 discovery and target verification.
- A further safety review found that isolated worker mode could otherwise execute a nonlocal URL. The default production runner now blocks such a target before browser connection; a direct test covers the refusal. The first 275-test local run had 274 passes and one outdated dashboard-copy assertion that still demanded the removed chat-created-target promise. The corrected assertion checks the real structured form and explicit guidance; targeted rerun passed. Full exact-head CI remains required after this change.
