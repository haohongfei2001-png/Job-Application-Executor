...
|---|---|---|
| C-01 | Authenticated fake site reaches unresolved fact with zero SMS sends and zero submits. A separate live-mode regression proves unverified account identity blocks before any form write; a certified site driver can provide positive proof. Real identity remains unverified. | `AUTO_PASS` for no-send and fail-closed synthetic paths; `FINAL_LIVE_NOT_RUN` |
...
| C-04–05 | Late fake source resumes a waiting task without owning a worker lease. Live continuation additionally requires the original process/tab/document and active OTP controls to pass read-only observation; old, expired, repeated, ambiguous and cross-task/attempt/origin codes fail closed. | `AUTO_PASS`; `FINAL_LIVE_NOT_RUN` |
...
| C-12 | Fake site redirect to a different job path becomes `auth_return_unverified`, blocks direct resume and does not re-send SMS or submit. Hash-routed job fragments are part of the exact return identity. | `AUTO_PASS` for wrong-route fault; `FINAL_LIVE_NOT_RUN` |
