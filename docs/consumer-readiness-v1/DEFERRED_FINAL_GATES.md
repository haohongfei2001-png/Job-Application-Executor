...
- **Current candidate engineering:** PR #20 re-observes each supported native control after reactive rendering before the next write. Value/choice reversion or control-contract changes raise ownership uncertainty and preserve the existing UNKNOWN_OUTCOME/no-replay worker fence. Passing per-field results are explicitly DOM_READBACK only, not server persistence or draft identity. Synthetic browser and real-worker regression coverage is pending exact-head targeted CI; this does not resolve DFG-002 or certify general ATS compatibility.
