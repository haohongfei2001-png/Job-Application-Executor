...
- **Safe degradation:** No real SMS, account, private Messages database, new permission or security challenge is used during unattended development. Local push requires a current task/attempt token; relay responses without attempt identity are ignored. Mac Messages is read only when an existing private configuration explicitly enables it, and a failed source does not make the task READY. Unsupported or ambiguous real authentication remains human-handled.
- **Existing evidence:** JCR-05 draft synthetic AuthAttempt/transport/browser tests and historical auth negative regressions; exact-head/main integration evidence is pending this round.
