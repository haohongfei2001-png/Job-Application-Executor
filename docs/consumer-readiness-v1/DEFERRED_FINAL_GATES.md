...
- **Existing evidence:** `tests/test_browser_runtime_v1.py` covers unrelated tab, unique owned popup, multiple popups, closed page, cross-origin page, process-epoch change, persisted crash attempt, durable tab/document binding, read-only page observation and no blind worker replay. One case uses actual isolated headless Chromium and a loopback HTTP fixture. `tests/test_consumer_entry_v1.py` verifies an independent loopback recovery page, its token/origin guard and isolated service restart without user Chrome. Local full suite after observation: 296 passed. PR #12 heads `34639e9`, `99e4bb3`, `60114c2` and `731d297` passed foundation and full test CI; observation-head CI pending.
