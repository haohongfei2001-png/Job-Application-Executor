...
tasksEl.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(!r.ok)throw new Error();
    input.value='';
    notify(remember?'答案已在本地保存为经确认的可复用事实。':'答案已在本地交给当前任务。');await state();
  }catch(e){
    try{
...
async function state(){
  try{
    ...
    if(!r.ok)throw new Error();
    const data=await r.json();
    if(![...tasksEl.querySelectorAll('.factinput input:not([type=checkbox])')].some(input=>input.value))render(data);
