"""Actual Vue 3 controlled input, rerender and independent draft oracle."""
...
BUNDLE = Path(__file__).parent / "fixtures" / "vue-controlled.bundle.js"


class VueFixture(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if (data.get("field") == "full_name" and isinstance(data.get("value"), str)
                and not self.server.drop_draft):
...
def test_vue_controlled_rerender_and_server_draft(tmp_path, monkeypatch):
    ats = ThreadingHTTPServer(("127.0.0.1", 0), VueFixture)
    ...
    try:
        with GenericWebAdapter(target) as adapter:
            ...
            actions = adapter.apply_resolutions([FieldResolution(
                ...
                status=ResolutionStatus.RESOLVED, value="Synthetic Vue Person")])
            ...
            assert adapter.page.locator("#framework-state").get_attribute(
                "data-framework-value") == "Synthetic Vue Person"
            assert adapter.page.locator("#name").input_value() == "Synthetic Vue Person"
        oracle = json.load(urllib.request.urlopen(
            f"http://127.0.0.1:{ats.server_port}/oracle"))
        assert oracle["draft"] == {"full_name": "Synthetic Vue Person"}
        assert oracle["revision"] >= 1
        assert oracle["submit_count"] == 0

        class VueFixtureAdapter(GenericWebAdapter):
            def verify_draft_persistence(self, _plan):
                observed = json.load(urllib.request.urlopen(
                    f"http://127.0.0.1:{ats.server_port}/oracle"))
                if observed["draft"] != {"full_name": "Synthetic Vue Person"}:
                    return None
            ...
            def observe_review_draft(self, plan):
                ...
                return {
                    ...
                    "draft_id_digest": digest("vue-fixture-draft"),
                    "revision": observed["revision"],
                    "account_verified": True,
                    "account_identity_digest": digest(
                        "identity.full_name:synthetic vue person"),
        ...
        profile.write_text(json.dumps({"fields": {"identity.full_name": {
            "value": "Synthetic Vue Person", "confidence": 1.0}}}))
        monkeypatch.setattr("executor.application.adapter_for_url",
                            lambda url: VueFixtureAdapter(url))
        ...
        assert VueFixtureAdapter(target).verify_draft_persistence(None) is None
