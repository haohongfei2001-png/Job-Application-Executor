...
def test_missing_or_future_sms_timestamp_fails_closed(tmp_path):
    db=tmp_path/'chat.db'; con=sqlite3.connect(db)
    con.executescript('CREATE TABLE handle(ROWID INTEGER PRIMARY KEY,id TEXT); CREATE TABLE message(ROWID INTEGER PRIMARY KEY,text TEXT,date INTEGER,handle_id INTEGER);')
    con.execute('INSERT INTO handle VALUES(1,?)',('1069',)); now=time.time()
    con.execute('INSERT INTO message(text,date,handle_id) VALUES(?,?,1)',('Code 111111',0))
    con.execute('INSERT INTO message(text,date,handle_id) VALUES(?,?,1)',
                ('Code 222222',int((now+60-APPLE_EPOCH)*1_000_000_000)))
